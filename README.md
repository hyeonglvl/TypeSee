...
- **틀린 단어 복습**: 세션에서 틀린 단어가 자동으로 모이고, 복습 노트에서 틀린 횟수 확인·재도전. TS-1 임계값에 도달하면 마스터 처리. 세션 카드 배지에 익힘 단계(점 5개) 표시
  - 게스트: localStorage에 백업되어 새로고침해도 유지
  - 로그인: `missed_words` 테이블에 저장되어 어디서든 복원 (ease factor 포함)
