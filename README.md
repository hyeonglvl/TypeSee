...
처음에는 SM-2(SuperMemo-2)와 FSRS 도입을 검토했지만, 둘 다 매일 접속을 전제로 한 날짜 기반
스케줄이라 가끔 들어오는 유저에게는 맞지 않았습니다. 그래서 직접 고안한 **TS-1**을 사용합니다.

**한 줄 요약**: 단어마다 ease factor를 매기고, 값이 낮을수록(=자주 틀릴수록) 세션에 더 자주 등장시킵니다.

- 맞히면 ease factor가 오르며 출현 확률이 낮아지고, 틀리면 반대로 움직입니다.
- 오래 안 본 단어는 시간 가중치가 붙어 출현 확률이 최대 3배까지 오르며(7일 포화), 날짜 스케줄 없이도 간격 효과(spacing effect)를 흉내 냅니다.
- 임계값을 넘기면 마스터로 졸업하되, 3일 뒤부터는 가끔 재등장시켜 유지 여부를 점검합니다 — 틀리면 복습 풀로 복귀합니다.
- 방금 맞힌 단어는 바로 다음 세션에서 한 번 쉬는 쿨다운을 둬, 맞히자마자 곧바로 다시 나오지 않게 합니다.
